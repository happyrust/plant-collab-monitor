...
| 编号 | 用例 | 期望 |
|---|---|---|
| LF-00 | 同 LR-00 + 快照 | 形状识别（2026-09-21 收紧）：`runtime/status` 带 `mode: standalone-real` / `running` / `relay` = **pws**，否则带 boolean `active` = pmg（pws ≥ 09-16 也带 boolean `active`，不能反过来先认 pmg）。记录联调前 `envIds`、运行态 `activeEnvId`、账面 `ledgerEnvId`、活动任务数；**pmg 且当前无激活 env** 时先 `POST envs/import-from-dboption` 生成快照 env（收尾用它 apply 回原 DbOption）；**pws** 记下 `GET /api/site/info` 的五个连接键（`ownConfig`，收尾写回用）——缺键或 `location_dbs` 为空（pws 不写空数组，激活后 API 写不回 `[]`）则**拒跑 full**、一个写请求都不发 |
| LF-01 | UI 登录 | 同 LR-01 |
| LF-02 | 新建测试 env `monitor-e2e-<stamp>` | 卡片出现；后端 `GET envs` 能查到；记录自动加入的本站站点 |
| LF-03 | 测 MQTT / 测文件服务 | 两条 banner 都出现、都不是 `请求失败`（结果按环境如实记录） |
| LF-04 | 激活 → 确定 | pill → `已激活 monitor-e2e-…`；后端运行态（`runtime/status.active && env_id`，pmg 与 pws ≥ 09-16 都有；没有运行态字段的旧 pws 退回账面 `envs.active.id`）== 测试 env；卡片徽标 + 按钮禁用；pws 的 activate 响应 `runtime_config`（`keys / changed`）被记进报告 `activations[]` 当「激活真改了文件」的证据 |
...
| LF-08 | 收尾恢复 | **直接删测试 env，靠后端级联删其站点**（pmg 一直如此；pws 2026-09-21 起，`afff42f`），删完回查 `GET envs/{id}/sites`——还剩站点 = 后端没级联，记 `noOrphanSites=false` 判失败，但仍把它们补删掉。**pws**（与 `topology-deploy-live-tutorial.mjs` 收尾同一套，2026-09-21 搬入）：本轮 activate 若 `runtime_config.changed === true` → 用开跑前 `site/info` 的五个键建临时恢复卡 `monitor-e2e-restore-<stamp>` 激活写回，再激活一次拿 `changed=false` 且 `keys` 含全部五键当凭证（`configRestored`；本轮没改过文件 = `null`）→ 运行态：开跑前在跑就重新 activate 那张，否则 `runtime/stop` → 删恢复卡 → 账面标记：开跑前有标记且不是运行态那张 → `apply` 标回去。**pmg**：原来有激活 env → 重新 activate；原来没有 → apply 快照 env + `runtime/stop`，再删快照 env。断言：env 集合与联调前一致、运行态恢复（`runtimeRestored`）、pws 账面标记恢复（`ledgerRestored`）、**无孤儿站点**、`configRestored !== false`；记录活动任务数前后（pws 的 activate / apply 任务记录不随 env 删除，会 +2～3，属已知后端行为） |

**2026-09-21 结果**（本机 Site A `:4100` plant-web-server 中继后端 · 隔离配置 `runtime/local-collab/site-a`，pws `afff42f`；脚本收紧前的一跑）：**9/9**，15 s，pageErrors 0。UI 建 `monitor-e2e-20260921-113055`（后端 `env-1789990258`，自动加入本站 `SCB` → `site-1789990258`）→ 测 MQTT `目标可达 · 127.0.0.1:1883`、测文件服务 `HTTP 200 · http://127.0.0.1:4101`（09-18 探测真探后首次在 LF 里两绿）→ 激活后 `active:true`、`mqtt_connected:true`、账面 `active.id == env-1789990258` → 应用成功 → 站点 test-http `HTTP 404`（本站 `http_host` 是监控台自己）+ 备注 PUT 落盘 → 停止后 `active:false`、`running:true` → 收尾 `DELETE envs/env-1789990258` 返回 `{deleted:true, deleted_site_count:1, deleted_sites:[site-1789990258]}`，回查站点 0、`sites.json` 为 `[]`，env 集合 / 激活态与跑前一致；活动任务 20 → 22。这一跑暴露两处、**同日晚已改**：① 09-16 起 pws 的 `runtime/status` 带 boolean `active`，脚本的形状识别把它认成 `pmg`，于是开跑前多发了一次 `import-from-dboption`（pws 只是覆盖同一张 `dboption-local-a` 卡，响应无顶层 `id` → `snapshotEnvId=null`，收尾没有误删）→ 识别改成先认 pws 记号（见 LF-00）；② 对 pws，激活把测试 env 的 `location / location_dbs / file_server_host` 写进了 `site-a/DbOption.toml`，脚本收尾**不回写**（教程脚本才回写），这次由人从跑前备份复原（SHA256 一致；进程内 `GET /api/site/info` 全程仍是 `local-a / [6000]`）→ 教程脚本那套「恢复卡激活写回 + 二次激活 `changed=false`」搬进 LF-08（见上表）。**改完复跑**（Site A 当时正被人拿来走 `/guide` 向导、有激活中的环境，不去打扰；另起一个隔离的临时 pws 实例 `:4199`——`location=local-t`、`location_dbs=[6100]`、独立 runtime 目录）：**9/9**，18 s，`shape: pws`，`apiCalls` 里不再有 `import-from-dboption`；页面 activate 响应 `runtime_config.changed=true, keys=五键` → 收尾 `create-restore-env` → `activate-restore-env`（`changed=true`）→ `activate-restore-env-verify`（**`changed=false`**，五键齐）→ `stop-runtime` → `delete-restore-env`（`deleted_site_count:0`）；`configRestored=true`、`runtimeRestored=true`、`ledgerRestored=true`，跑完该实例的 `DbOption.toml` **SHA256 与跑前一致**、envs 空、`sites.json` 为 `[]`。对 Site B `:4101`（`location_dbs=[]`）试 full → LF-00 后即拒跑「写不回 `[]`」，0 写请求、文件 SHA 不变。**Site A 空下来后（20:22）再跑一次：9/9**，`monitor-e2e-20260921-122220` → `env-1789993343`，activate `changed=true` → 收尾恢复卡 `env-1789993354` 激活写回、二次激活 `changed=false` → stop → 删恢复卡；`site-a/DbOption.toml` 跑前跑后 SHA256 一致（`7213…E5FB`），envs 回到 1 张登记卡、`sites.json=[]`，活动任务 24 → 27（activate ×2 + apply）——**结果 JSON 就是这一跑**。
