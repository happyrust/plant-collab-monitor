...
14. ~~**pws `GET /api/site/info` / `site-config` 只回启动快照**（激活改了 toml 之后还是旧值，向导第 8 步「文件是否被改过」永远「一致」，教程 / smoke 记的「原值」其实是启动值）~~ → 2026-09-21 晚已修（pws `ac299df`：五个连接键每次重读文件；监控台同日 fix(guide) 第 8 步改逐键对照表 + 后端太旧提示）。**部署提醒**：Site A / B 现在跑的还是旧 exe（两站进程占着 `D:\Rust\target\debug\plant-web-server.exe`，`cargo build` 替换不了），下次重起两站前先 `cargo +nightly-2026-07-21 build --bin plant-web-server`，否则 `/guide` 第 8 步在 Site A 上会看到红字「site/info 不重读文件」。
