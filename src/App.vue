        ...
        >
          ...
          <nav class="flex-1 py-6 space-y-1 text-sm overflow-y-auto" :class="sidebarCollapsed ? 'px-1' : 'px-3'">
            ...
            <RouterLink
              ...
              custom
            >
              <NTooltip :disabled="!sidebarCollapsed" placement="right" :show-arrow="false">
                <template #trigger>
                  <button
            ...
            >
              <NTooltip :disabled="!sidebarCollapsed" placement="right" :show-arrow="false">
                ...
                {{ item.label }}{{ item.admin ? ' 🔒' : '' }}{{ item.name === 'tasks' && appStatus.queue.failed > 0 ? ` (${appStatus.queue.failed} 失败)` : '' }}
              </NTooltip>
            </RouterLink>
