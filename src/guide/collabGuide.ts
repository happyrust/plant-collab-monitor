...
export const COLLAB_GUIDE_STEPS: GuideStepDef[] = [
  ...
  },
  {
    ...
    checkHint: '本页「帮我测」或页面上两次探测都返回可达（本会话内记录）。',
    checkable: true,
    route: '/topology',
    tour: [
  ...
  },
  {
    ...
    why: '右表登记的是对端节点，给拓扑图、探测和「查看站点详情」用。保存环境时自动加进来的那个站点指向的是你自己（监控台地址），第一次探测必然 404 —— 真要登记对端必须手动改成对端地址。',
    howto: [
      '选中环境，看右侧站点表：自动加入的「本站」那行，点行尾「听诊器」探测 → 预期红：HTTP 404 · <监控台地址>/metadata.json。',
    ...
    checkHint: '激活 / 选中的环境下 sites ≥ 1，且本页或页面上探测过一次可达。',
    checkable: true,
    route: '/topology',
    tour: [
      T('site-test-http', '探测这一行', '让后端对这个站点的 http_host 拼上 /metadata.json 发一次 GET。自动加进来的「本站」行第一次必然不通（指向监控台自己）。', { missingHint: '先在左侧选中一个环境；该环境下还没有站点就先点「添加站点」。' }),
