...
- 验证：Site A 当时正被人拿来走 `/guide` 向导（有激活中的环境），不去打扰——另起一个隔离的临时 pws 实例 `:4199`（`location=local-t`、`location_dbs=[6100]`、独立 `PLANT_WEB_RUNTIME_DIR`）跑 `--mode full --confirm-writes` → **9/9**，18 s，`shape: pws`，`apiCalls` 无 `import-from-dboption`；activate `changed=true, keys=五键` → 收尾 `activate-restore-env`（`changed=true`）→ `activate-restore-env-verify`（**`changed=false`**）→ `stop-runtime` → `delete-restore-env`；`configRestored / runtimeRestored / ledgerRestored` 全 true，跑完该实例 `DbOption.toml` SHA256 与跑前一致、envs 空、`sites.json` 为 `[]`（实例随后已杀、目录已删）。对 Site B `:4101`（`location_dbs=[]`）试 full → LF-00 后即拒跑，0 写请求、文件 SHA 不变。Site A 空下来后再对它跑一次 → **9/9**，`site-a/DbOption.toml` 自己写回、SHA256 与跑前一致，活动任务 24 → 27（结果 JSON 就是这一跑）。`node --check` 通过。
...
- 留：`sync_ledger` / `mqtt_file_sync` / `SyncE3dFileMsg`（含 `file_sesnos`）——完整站点仍要收发 MQTT 源文件并记这本账，线格式也必须跟 `plant-web-server` 保持兼容。没有 e3d-io 的构建里收包端只比 hash，sesno 一项记 `skipped`（这条分支本来就在）。
- `cargo check --bin web_server --features web_server,mqtt` 通过（2 m 45 s），删后本仓新增警告 0；删完再跑一次 24 项，仍 24/24。

#### Changed · 24 项双站点 smoke 换到新后端，连跑三次 24/24

- `scripts/local-remote-collab-setup.ps1`：生成的 `start.ps1` 改起 `plant-web-server.exe`，带 `--repo-root` 与每站一个的 `PLANT_WEB_RUNTIME_DIR`；前置检查与 `COMMANDS.md` 的构建提示同步改成 `cargo build --bin plant-web-server`。
